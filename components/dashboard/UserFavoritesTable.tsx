...
export function UserFavoritesTable({ favorites: initialFavorites }: { favorites: FavoriteRow[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return initialFavorites.filter((f) => f.gameName.toLowerCase().includes(q));
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {filtered.length > 0 && (
          <span className="text-sm text-muted-foreground">
            {filtered.length} of {initialFavorites.length} games
          </span>
        )}
      </div>
        ...
        <table className="w-full text-sm table-fixed">
          <thead>
            <tr className="border-b border-border text-muted-foreground">
              <th className="w-[22%] px-2 py-3 text-left font-medium">Game</th>
              <th className="w-[18%] px-2 py-3 text-left font-medium">Genre</th>
              <th className="w-[22%] px-2 py-3 text-left font-medium">Platforms</th>
              <th className="w-[18%] px-2 py-3 text-left font-medium">Publisher</th>
              <th className="w-[10%] px-2 py-3 text-center font-medium">Rating</th>
              <th className="w-[10%] px-2 py-3 text-left font-medium">Saved</th>
            </tr>
          </thead>
          <tbody>
            {paged.map((f) => (
